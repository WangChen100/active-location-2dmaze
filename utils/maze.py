...
def get_depth(map_design, position, orientation):
    '''
    Return depth, no wall, with given position and orientation 
    '''
    m, n = map_design.shape
    depth = 0
    new_tuple = position
    # ensure position in the map
...
def get_next_state(map_design, position, orientation, action):
    '''
    Return new location and orientation number
    '''
    m, n = map_design.shape
    if action == 'TURN_LEFT' or action == 0:
        ...
        new_tuple = get_tuple(position, orientation) #new location
        # ensure new location inside map
...
def get_random_position(map_design):
    '''
    Return a random location not in wall
    '''
    m, n = map_design.shape
    while True:
...
def get_tuple(i, orientation):
    '''
    Return a new location tuple alone given orientation
    '''
