...
def transition_function(belief_map, depth, action):
    '''
    Return a new belief map after action
    '''
...
def get_all_likelihoods(map_design):
    '''
    Return binary all_likelihoods matrix. 1 indicates depth
    '''
    num_orientations = 4
    all_likelihoods = np.zeros(
        [map_design.shape[0] - 2, num_orientations,
         map_design.shape[0], map_design.shape[1]]) # map_design.shape[0]-2 means depth belongs to [0,5]
    # go through all orientations
    for orientation in range(num_orientations):
        # go through all localtion in map
...
def shift_belief(B, orientation):
    '''
    Return a shift belief map after action
    '''
    if orientation == 0 or orientation == "east":
        Bcap = np.insert(
            B, 0, np.zeros(
                B.shape[0]), axis=1)#.reshape(B.shape[0], B.shape[1] + 1)
        Bcap = np.delete(Bcap, -1, axis=1)
    elif orientation == 2 or orientation == "west":
        Bcap = np.append(B, np.zeros([B.shape[0], 1]), axis=1)#.reshape(B.shape[0], B.shape[1] + 1)
        Bcap = np.delete(Bcap, 0, axis=1)
    elif orientation == 1 or orientation == "north":
        Bcap = np.append(B, np.zeros([1, B.shape[1]]), axis=0)#.reshape(B.shape[0] + 1, B.shape[1])
        Bcap = np.delete(Bcap, 0, axis=0)
    elif orientation == 3 or orientation == "south":
        Bcap = np.insert(B, 0, np.zeros(B.shape[1]), axis=0)#.reshape(B.shape[0] + 1, B.shape[1])
